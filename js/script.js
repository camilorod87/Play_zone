window.onload = function()
{
	...
	var elementos = [ "identifica", "nombre", "apellido", "email", "fechanace", "campo", "estado"];
	var resultadoBusca = []; //Gurda los usuarios que cumplen con el criterio de búsqueda...
	//Constructor Persona...
	function persona(id, pn, pa, em, fe, cm, es)
	{
	...
	if(localStorage.getItem("listado"))
	{
		...
		for(var i in objTMP)
		{
			...
			var cm = objTMP[i].numerocampo;
			var es = objTMP[i].estadocampo;
			var nuevaPersona = new persona(id, pn, pa, em, fn, cm, es);
	...
	function imprimeUsuarios()
	{
		...
		var txt = "<table class = 'table-fill'>";
			txt += "<thead><tr><th>ID</th><th>Nombre</th><th>Email</th>";
			txt += "<th>Fecha</th><th>Edad</th><th>Campo</th><th>Estado</th>";
		...
		for(var i = 0; i < listadoPersonas.length; i++)
		{
			...
			if(muestra)
			{
				...
				nom_div("e_" + i).addEventListener('click', function(event)
				{
				    ...
				    nom_div("campo").value=listadoPersonas[ind].numerocampo;
				    nom_div("estado").value=listadoPersonas[ind].estadocampo;
	...
	nom_div("guarda").addEventListener('click', function(event)
	{
		...
		if(correcto)
		{
			...
			if(existeDatos == 0) //No existe...
			{
				...
				{
					var nuevaPersona = new persona(valores[0], valores[1], valores[2], valores[3], valores[4], valores[5], valores[6	]);
					listadoPersonas.push(nuevaPersona); 
	...
	//Función que valida que un e-mail tenga el simbolo @ y .com
